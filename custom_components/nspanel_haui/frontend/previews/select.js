  ...
  return {
    content: html`
      <div style="display:${isFull ? 'flex' : 'grid'};flex-direction:column;${isFull ? '' : 'grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(4,1fr)'};gap:3px;width:100%;flex:1;min-height:0;">
        ${isFull
          ? labels.map(l => html`
              <div class="pg-preview-btn active" style="width:100%;flex:1;min-height:0;"><span class="pg-preview-btn-label" style="font-size:0.5em;color:var(--primary-color,#4fc3f7);">${l}</span></div>
            `)
          : labels.map(l => html`
              <div class="pg-preview-btn active" style="width:100%;height:100%;"><span style="font-size:0.5em;color:var(--primary-color,#4fc3f7);">${l}</span></div>
            `)}
      </div>`,
