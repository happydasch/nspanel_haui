...
export function renderWeatherPreview(_host, panel, _pIdx, _pt) {
  ...
  const forecastType = panel?.forecast_type || '';
  const showForecast = forecastType && forecastType !== '';
  const isMono = panel?.weather_icons === 'monochrome';
  const iconColor = isMono
    ? 'var(--primary-text-color,#ddd)'
    : 'var(--primary-color,#4fc3f7)';
  return {
    content: html`
          ...
          <div class="pg-preview-forecast">
            ${[t('Mon'),t('Tue'),t('Wed'),t('Thu'),t('Fri')].map((day, i) => html`
              <div class="pg-preview-forecast-day">
                ...
                <ha-icon icon="${['mdi:weather-sunny','mdi:weather-cloudy','mdi:weather-rainy','mdi:weather-partly-cloudy','mdi:weather-sunny'][i]}" style="${!isMono ? 'color:var(--primary-color,#4fc3f7);' : ''}"></ha-icon>
