...
class HAUINavigationController(HAUIBase):
    ...
    def __init__(self, app, config):
        ...
        self._stack: list[
            tuple[HAUIPanel, dict[str, Any]]
        ] = []  # stack for non-nav panels
        self._snapshot = None  # snapshot for navigation
        self._snapshot_time: float | None = (
            None  # monotonic time when snapshot was created
        )
    ...
    def open_home_panel(self, autostart: bool = False):
    ...
    def process_event(self, event: HAUIEvent):
        ...
        if event.name == ESP_EVENT["timeout"] and event.value == "page":
            self._sleep_panel_active = True
        ...
        # sleep handling
        if event.name == ESP_EVENT["sleep"] and self.page:
            # unset current page
            self.unset_page()
