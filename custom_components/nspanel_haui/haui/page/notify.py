...
class NotifsPage(CommonNotifyPage):
    ...
    def render_panel(self, panel: HAUIPanel) -> None:
        ...
        if not notifications:
            self.set_component_text(
                self.TXT_TEXT_FULL, self.translate("No notifications")
            )
            self.hide_component(self.TXT_TEXT)
            self.hide_component(self.TXT_ICON)
            self.show_component(self.TXT_TEXT_FULL)
    ...
    def callback_button(self, event, component, button_state) -> None:
        ...
        if component == self.BTN_RIGHT:
            ...
            if notifications:
                self._index = max(0, min(self._index, len(notifications) - 1))
                self.app.controller["notification"].remove_notification(
                    notifications[self._index]
                )
            ...
            if self.panel is not None:
                ...
                with self.rec_cmd:
                    ...
                    self.update_function_component(
                        self.FNC_BTN_L_SEC, visible=count > 1
                    )
                    self.update_function_component(
                        self.FNC_BTN_R_SEC, visible=count > 1
                    )
