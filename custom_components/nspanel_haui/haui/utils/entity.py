...
def _color_light(entity, entity_state, haui_entity):
    ...
    if "brightness" in attr:
        return rgb_to_rgb565(
            rgb_brightness(rgb565_to_rgb(COLORS["entity_on"]), attr["brightness"])
        )
    return None
...
def _value_lock(entity, entity_state, haui_entity):
    return (
        haui_entity.translate("Lock")
        if entity.state == "unlocked"
        else haui_entity.translate("Unlock")
    )
...
def _value_vacuum(entity, entity_state, haui_entity):
    return (
        haui_entity.translate("Clean")
        if entity.state == "docked"
        else haui_entity.translate("Stop")
    )
