...
def _color_weather(entity: Any, item_state: str | None, haui_item: HAUIItem) -> int:
    color_name = f"weather_{(item_state or '').replace('-', '_')}"
    return COLORS.get(color_name, COLORS["weather_default"])


def _color_climate(entity: Any, item_state: str | None, haui_item: HAUIItem) -> int | None:
    return COLORS.get(f"climate_{item_state}")


def _color_alarm(entity: Any, item_state: str | None, haui_item: HAUIItem) -> int | None:
...
def _color_media_player(entity: Any, item_state: str | None, haui_item: HAUIItem) -> int:
    if item_state == "playing":
        return COLORS["item_on"]
    if item_state == "unavailable":
        return COLORS["item_unavailable"]
    return COLORS["item_off"]


def _color_group(entity: Any, item_state: str | None, haui_item: HAUIItem) -> int:
    return COLORS["item_on"] if item_state == "on" else COLORS["item_off"]


def _color_light(entity: Any, item_state: str | None, haui_item: HAUIItem) -> int | None:
    if item_state != "on":
        return None
    ...
    if "brightness" in attr:
        return rgb_to_rgb565(rgb_brightness(rgb565_to_rgb(COLORS["item_on"]), attr["brightness"]))
    return None
...
def get_item_color(haui_item: HAUIItem, default_color: int) -> int:
    ...
    if item_state in _ON_STATES:
        result_color = COLORS["item_on"]
    elif item_state == "unavailable":
        result_color = COLORS["item_unavailable"]
    else:
        result_color = COLORS["item_off"]

    # per-type override
    handler = _COLOR_DISPATCH.get(item_type)
    if handler:
        color = handler(entity, item_state, haui_item)
